...
const EVENT_DAY_FORMATTER = new Intl.DateTimeFormat('en-CA', {
...
function toEventDateKey(value: any): string | null {
    const date = toDate(value)
    if (!date) return null

    const parts = EVENT_DAY_FORMATTER.formatToParts(date)
...
function getLastDayIndex(sessionData: any): number {
    const startKey = toEventDateKey(sessionData.start_date)
    const endKey = toEventDateKey(sessionData.end_date) || startKey
    if (!startKey || !endKey) return 0

    return Math.max(0, Math.round((dateKeyToUtcTime(endKey) - dateKeyToUtcTime(startKey)) / (1000 * 60 * 60 * 24)))
}
