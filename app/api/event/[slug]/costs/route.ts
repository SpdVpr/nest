...
export async function GET(
    ...
    try {
        ...
        const hwByGuest: Record<string, { name: string; qty: number; nights_count: number; totalPrice: number; type: string }[]> = {}
        await Promise.all(
            hwReservationsSnapshot.docs.map(async (doc) => {
                ...
                hwByGuest[guestId].push({
                    ...
                    qty: data.quantity || 1,
                    nights_count: data.nights_count || 1,
        ...
        const guestCosts = await Promise.all(
            guestsSnapshot.docs.map(async (guestDoc) => {
                ...
                return {
                    ...
                    deposit: guestData.deposit || 0,
                    grandTotal: nightsTotal + snacksTotal + hwTotal + tip.amount,
                    // Settlement info (only when settlement has been finalized by admin)
                    settlement: settlement && settlement.qr_generated_at ? {
                        ...
                        payment_method: settlement.payment_method || 'qr',
